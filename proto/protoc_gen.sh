...
# 3. 用于生成pb文件
protoc --proto_path=${INPUT_PROTO} --go_out=plugins=grpc:. ${INPUT_PROTO_FILE}
# 4. 用于生成pb文件，pb.go文件， gateway文件
protoc --proto_path=${INPUT_PROTO} --go_out . --go-grpc_out . --grpc-gateway_out . ${INPUT_PROTO_FILE}
